...
def constructions_total_cost(project_document_type=None):
    monthly_expense = MonthlyExpense.objects.none() 
    if project_document_type:
        monthly_expense = MonthlyExpense.objects.filter(construction_task__project_document_type=project_document_type)
    total_cost = monthly_expense.aggregate(total_cost=Sum('construction_task__total_cost', distinct=True))['total_cost']
    return total_cost or 0



def constructions_total_cost_for_month(queryset, project_document_type=None):
    if project_document_type:
        queryset = queryset.filter(construction_task__project_document_type=project_document_type)
...
def get_total_year_sum(queryset, project_document_type=None):
    if project_document_type:
        queryset = queryset.filter(construction_task__project_document_type=project_document_type)
...
def total_year_calculation_horizontally(queryset, project_document_type=None):
...
def get_fact_sum(queryset, project_document_type=None):
    if project_document_type:
        queryset = queryset.filter(construction_task__project_document_type=project_document_type)
...
def get_total_fact_sum(queryset, project_document_type=None):
    fact_sums = get_fact_sum(queryset, project_document_type)
    total = sum(item['total_spent'] for item in fact_sums)
    return total


def get_difference(queryset, project_document_type=None):
    fact_sum = get_fact_sum(queryset, project_document_type)
...
def get_total_difference(queryset, project_document_type=None):
    differences = get_difference(queryset, project_document_type)
