...
class ProjectSection(BaseModel):
    project_document_file = models.ForeignKey(ProjectDocumentType, on_delete=models.SET_NULL, verbose_name="Keyingi hujjat", blank=True, null=True)
    name = models.CharField(max_length=1000, blank=True, null=True, verbose_name="Nomi")

    def __str__(self):
        return f"{self.project_document_file} -- {self.name}"
